<!-- This explains "HOW do I release ESO" -->
<!-- It is not a place to clarify/explain why/schedule of our releases (policy.md) -->

# Releasing how-to

ESO and the ESO Helm Chart have two distinct lifecycles and can be released independently.
Helm Chart releases are named `external-secrets-x.y.z`.
...
**All modules share the same version tag.**

When releasing version `v0.x.y`, a single git tag is created that applies to all modules in the repository. Go's module system automatically handles this, and consumers can reference any module using the same version tag.
...
// TODO EVRARDJP: We need to create this branch first, right? Check with skarlso

The branch to create a release should be `release-chart-x.y.z`. Though be aware that release branches are _immutable_.
...
2. push to branch and open pr. Double-check the following:
   1. helm docs were updated (TODO EVRARDJP)
   2. apiVersion in the snapshots are correct (TODO EVRARDJP)
   3. Update all the helm tests with potential added values (TODO EVRARDJP)
   4. Update the stability docs with the latest minor version if exists (TODO EVRARDJP)
   5. Update the CRD conformance tests (TODO EVRARDJP)
